...
class Controller extends CController
{
    ...
    public function readFileFromInput($prefix = ""){
        $ext = ".xml";
        $file = file_get_contents("php://input");
        // $file = "safasdfas";
        if( empty($file) ){
            Debug::log("Пустое тело запроса ".$filename, true);
            die();
        }

        do {
            $filename = Yii::app()->basePath."/../1c_exchange/" . $prefix . '_' . date( DATE_W3C ) . $ext;
        } while ( file_exists( $filename ) );

        if ( !file_put_contents( $filename, $file ) ){
            Debug::log("Ошибка записи в файл ".$filename, true);
            die();
        }else{
            Debug::log("Файл успешно записан ".$filename, true);
        }
    ...
    public function returnXMLFile($name, $output){
        header('Content-Description: File Transfer');
        header('Content-Type: application/force-download');
        header('Content-Type: application/octet-stream');
        header('Content-Type: application/download');
        header('Content-Disposition: attachment; filename=' . $name . "_" . date('d-m-y_H-i-s') . '.xml' );
        header('Content-Transfer-Encoding: binary');
        header('Expires: 0');
        header('Cache-Control: must-revalidate');
        header('Pragma: public');
