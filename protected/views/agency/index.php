...
<? if( Yii::app()->user->checkAccess('root') ): ?><a href="<?php echo $this->createUrl("/".$this->adminMenu["cur"]->code."/create")?>" class="ajax-form ajax-create b-butt b-top-butt">Добавить</a><? endif; ?>
	...
	<? if(count($data)): ?>
		<? foreach ($data as $i => $item): ?>
			<tr>
				...
				<td>
					<? if( Yii::app()->user->checkAccess('updateDictionary') ): ?><a href="<?php echo Yii::app()->createUrl('/'.$this->adminMenu["cur"]->code.'/update',array('id'=>$item->id))?>" class="ajax-form b-double-click-click ajax-update b-tool b-tool-update" title="Редактировать <?=$this->adminMenu["cur"]->vin_name?>"></a>
					<? if( Yii::app()->user->checkAccess('root') ): ?><a href="<?=Yii::app()->createUrl('/'.$this->adminMenu["cur"]->code.'/delete',array('id'=>$item->id))?>" data-name="<?=$this->adminMenu["cur"]->vin_name?>" class="ajax-form ajax-delete b-tool b-tool-delete" title="Удалить <?=$this->adminMenu["cur"]->vin_name?>"></a><? endif; ?><? endif; ?>
				</td>
			</tr>
		<? endforeach; ?>
	<? else: ?>
